...
export default function BranchManagementPage() {
  ...
  const handleStatusChange = async (branchId: string, action: 'activate' | 'deactivate') => {
    try {
      ...
      setBranches(branches.map(branch => 
        branch.id === branchId 
          ? { ...branch, status: action === 'activate' ? 'active' : 'inactive' } 
          : branch
      ));
    } catch (err) {
      console.error(`Error ${action}ing branch:`, err);
      alert(`Gagal ${action === 'activate' ? 'mengaktifkan' : 'menonaktifkan'} cabang. Silakan coba lagi.`);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Manajemen Data Cabang</h2>
        </div>
        <Button 
      ...
      {loading ? (
        <div className="flex justify-center items-center h-64">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#1E8F59]"></div>
        </div>
      ) : error ? (
                    ...
                    <TableCell className="pr-8">
                      <div className="flex justify-end">
                        <Button 
                          ...
                          asChild
                        >
                          <Link href={`/dashboard/branch-data/edit/${branch.id}`}>
