      ...
      <div className="flex items-center gap-2">
        {isStatus ? (
          <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-3 py-1 text-xs font-medium text-red-700">
            <Icon icon="lucide:x-circle" width={14} height={14} />Gagal
          </span>
        ) : (
          <>
...
function FailedPageContent() {
  ...
  useEffect(() => {
    const validateAndLoadData = async () => {
      ...
      try {
        ...
        if (!result.ok || !result.data?.order) { setOrderNotFound(true); setLoading(false); return; }
      ...
      } catch (error) { console.error("Error fetching order:", error); setOrderNotFound(true); }
  ...
  if (orderNotFound) return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-emerald-50/50 p-4">
      ...
      <button onClick={() => navigate.push("/cart")} className="rounded-lg bg-emerald-700 px-4 py-2 text-white hover:bg-emerald-800">Kembali ke Keranjang</button>
    </div>
  );
    ...
    <div className="flex min-h-screen flex-col bg-gray-50">
      ...
      <main className="flex flex-grow items-center justify-center bg-emerald-50/50 px-4 py-10">
        <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl">
          ...
          <h1 className="mb-6 text-center text-2xl font-bold text-gray-900">Pembayaran Gagal</h1>
          <div className="mb-6 rounded-xl bg-red-50 p-5">
            <div className="mb-4 flex items-center justify-between border-b border-red-200 pb-3">
              ...
              <span className="text-xl font-bold text-red-500">{currency(grand_total).replace("Rp", "Rp.")}</span>
            </div>
          ...
          <div className="mb-6 flex items-start gap-3 rounded-xl bg-amber-50 p-4">
            <div><p className="text-center text-xs text-amber-700">Silahkan buat pesanan baru jika Anda masih ingin melanjutkan pembelian</p></div>
          </div>
...
export default function FailedPage() {
  return <Suspense fallback={<div className="flex min-h-screen items-center justify-center"><p>Mengalihkan...</p></div>}><FailedPageContent /></Suspense>;
}
