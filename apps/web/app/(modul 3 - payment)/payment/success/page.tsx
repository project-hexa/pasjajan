...
const DetailRow: React.FC<{ label: string; value: string; isCopyable?: boolean; isStatus?: boolean }> = ({ label, value, isCopyable, isStatus }) => {
      ...
      <div className="flex items-center gap-2">
        {isStatus ? (
          <span className="inline-flex items-center gap-1 rounded-full bg-green-100 px-3 py-1 text-xs font-medium text-green-700">
            <Icon icon="lucide:check-circle" width={14} height={14} />Lunas
          </span>
        ) : (
          <>
            {isCopyable && <button title={copied ? "Tersalin!" : "Salin"} onClick={handleCopy} className="p-1 text-gray-400 hover:text-emerald-700"><Icon icon="lucide:copy" width={14} height={14} /></button>}
    ...
    <div className="flex min-h-screen flex-col bg-gray-50">
      ...
      <main className="flex flex-grow items-center justify-center bg-emerald-50/50 px-4 py-10">
        <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl">
          ...
          <h1 className="mb-6 text-center text-2xl font-bold text-gray-900">Pembayaran Berhasil!</h1>
          <div className="mb-6 rounded-xl bg-emerald-50 p-5">
            <div className="mb-4 flex items-center justify-between border-b border-emerald-200 pb-3">
              ...
              <span className="text-xl font-bold text-emerald-600">{currency(orderData.grand_total).replace("Rp", "Rp.")}</span>
            </div>
            <div className="space-y-1">
              {orderData.payment_method && <DetailRow label="Metode Pembayaran" value={orderData.payment_method.name} />}
          ...
          <div className="mb-6 flex items-start gap-3 rounded-xl bg-blue-50 p-4">
            <div className="flex-shrink-0 rounded-full bg-blue-100 p-2"><Icon icon="lucide:package" width={20} height={20} className="text-blue-600" /></div>
            <div>
              <p className="mb-1 text-sm font-medium text-blue-900">Pesanan Anda Sedang Diproses</p>
              <p className="text-xs text-blue-700">Kami akan segera memproses pesanan Anda. Detail pemesanan telah dikirim ke email Anda</p>
            </div>
          </div>
...
export default function SuccessPage() {
  return <Suspense fallback={<div className="flex min-h-screen items-center justify-center"><p>Mengalihkan...</p></div>}><SuccessPageContent /></Suspense>;
}
